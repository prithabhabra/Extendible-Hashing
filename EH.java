    ...
    static void directory_doubling(ArrayList<Directory> d){
        ...
        for(int i=0;i<Math.pow(2,dir)/2;i++){
            if(i!=index){
                Block b=new Block();
                d.add(new Directory(String.format(format, Integer.toBinaryString(i)).replace(" ", "0"), b));
                d.add(new Directory(String.format(format, Integer.toBinaryString(i+4)).replace(" ", "0"), b));
                b.block=dir-1;
            } 
            else{
                Block b1=new Block();
                ...
                b1.block=dir;
                Block b2=new Block();
                d.add(new Directory(String.format(format, Integer.toBinaryString(i+4)).replace(" ", "0"), b2));
                b2.block=dir;
            }
        }
        insert(keys,d);
